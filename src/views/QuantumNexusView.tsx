...
interface QuantumNexusViewProps {
  onBackToPackages?: () => void;
  onBackToDashboard?: () => void;
  onNavigateToNexusPrime?: () => void;
  isUserPackageActive?: boolean;
  onOpenPackageModal?: () => void;
}

export const QuantumNexusView: React.FC<QuantumNexusViewProps> = ({
  onBackToPackages,
  onBackToDashboard,
  onNavigateToNexusPrime,
  isUserPackageActive = true,
  onOpenPackageModal,
}) => {
  // STRICTLY SCOPED TO PACKAGE ID 1 (Quantum Node, $70)
  const PACKAGE_ID = 1;
  ...
  const isAccessible = programPhaseService.isTreeAccessible('quantum_nexus', isUserPackageActive);
  const operationalState = programPhaseService.getModuleStatus('quantum_nexus', isUserPackageActive);
  ...
  // Load contract-derived state specifically for Quantum Node (pkgId = 1)
  useEffect(() => {
    let isMounted = true;
    setLoading(true);
    ...
    async function loadQuantumPackageData() {
      try {
        ...
        console.error('Failed to read contract data for Quantum Node package 1', err);
        if (isMounted) setLoading(false);
      }
    }

    loadQuantumPackageData();
  ...
  if (loading || !treeData || !stats) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
        <div className="w-12 h-12 rounded-2xl bg-emerald-950/80 border border-emerald-500/40 flex items-center justify-center animate-spin text-emerald-400">
          <Network className="w-6 h-6" />
        </div>
        <p className="text-xs font-mono text-zinc-400">
          Reading Quantum Node ($70) contract telemetry...
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300 pb-12">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-emerald-950 text-emerald-300 border border-emerald-500/40 uppercase">
                Package 3 &bull; Contract ID: 1
              </span>
              <span className="text-xs font-mono text-zinc-400">
                Independent Quantum Node Module
              </span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-black text-white font-mono tracking-tight flex items-center gap-2.5 mt-0.5">
              ...
              <span>Quantum Node X6 Matrix</span>
            </h1>
          </div>
        </div>

        {/* Action button to switch to Nexus Prime Node ($120) */}
        {onNavigateToNexusPrime && (
          <button
            onClick={onNavigateToNexusPrime}
            className="px-4 py-2.5 rounded-2xl bg-zinc-900/90 hover:bg-zinc-800 text-zinc-300 hover:text-white border border-zinc-700 hover:border-teal-500/50 transition-all font-mono text-xs font-bold flex items-center gap-2.5 self-start md:self-auto cursor-pointer shadow-md group"
          >
            <Sparkles className="w-4 h-4 text-teal-400 group-hover:scale-110 transition-transform" />
            <span>Open Nexus Prime Node ($120)</span>
      ...
      <div className="rounded-3xl bg-gradient-to-r from-emerald-950/50 via-zinc-950 to-[#07130c] border border-emerald-500/30 p-6 sm:p-7 relative overflow-hidden backdrop-blur-xl shadow-xl">
        <div className="absolute right-0 top-0 w-80 h-80 bg-emerald-500/10 rounded-full blur-3xl pointer-events-none" />
        
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="flex items-center gap-2.5 flex-wrap">
              <span className="text-xs font-mono font-bold uppercase tracking-wider text-emerald-400 px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-500/40 flex items-center gap-1.5">
          ...
          <div className="flex items-center gap-4 flex-wrap shrink-0">
            <div className="p-4 rounded-2xl bg-zinc-900/80 border border-emerald-500/20 text-center min-w-[110px]">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
        ...
        <div className="p-3.5 rounded-2xl bg-zinc-950/80 border border-zinc-800/80">
          ...
          <span className="text-lg font-black text-zinc-300 font-mono">
            {stats.openPositions ?? stats.availablePositions ?? 11}
          </span>
          <span className="block text-[9px] font-mono text-zinc-500 mt-0.5">
            Available Positions
          </span>
        ...
        <div className="p-3.5 rounded-2xl bg-zinc-950/80 border border-zinc-800/80">
          ...
          <span className="text-lg font-black text-emerald-400 font-mono">
            {stats.directPlacements ?? stats.directMembersCount ?? 7}
          </span>
          <span className="block text-[9px] font-mono text-zinc-500 mt-0.5">
            Sponsored Nodes
          </span>
        ...
        <div className="p-3.5 rounded-2xl bg-zinc-950/80 border border-zinc-800/80">
          ...
          <span className="text-lg font-black text-pink-400 font-mono">
            {stats.spilloverCount ?? stats.spilloverMembersCount ?? 8}
          </span>
          <span className="block text-[9px] font-mono text-zinc-500 mt-0.5">
            Upline Placements
          </span>
        ...
        <div className="p-3.5 rounded-2xl bg-zinc-950/80 border border-zinc-800/80">
          ...
          <span className="text-lg font-black text-emerald-300 font-mono">
            ${(stats.matrixIncomeUSD ?? 14).toFixed(2)}
          </span>
          <span className="block text-[9px] font-mono text-zinc-500 mt-0.5">
            Earned to Date
          </span>
          ...
          <div>
            ...
            <h3 className="text-xl font-bold text-white mt-1">
              {operationalState === 'COMING_SOON'
                ? 'Quantum Matrix Mainnet Launch Pending'
                : 'Quantum Node Activation Required'}
            </h3>
            <p className="text-xs text-zinc-400 max-w-md mx-auto mt-2 leading-relaxed">
              {operationalState === 'COMING_SOON'
                ? 'The Quantum 4-Ring Matrix is scheduled for activation in Phase 3 of the MDeFi protocol rollout. Real-time matrix tree visualization and cashback rewards will unlock upon deployment.'
                : 'Activate the $70 Quantum Node to unlock your 4-Ring concentric matrix tree, instant cashback tracking, and generation reward positions.'}
            </p>
          </div>
