...
import { WeeklyStarterData, WeeklyStarterClaimRecord } from '../types/rewards';
...
interface WeeklyRewardStarterViewProps {
  user: UserProfile;
...
export const WeeklyRewardStarterView: React.FC<WeeklyRewardStarterViewProps> = ({
  user,
  rewards,
  onUpdateRewards,
  onShowToast,
  ...
  const [claims, setClaims] = useState<WeeklyStarterClaimRecord[]>([]);
  const [loading, setLoading] = useState(true);
  const [copiedTx, setCopiedTx] = useState<string | null>(null);

  // Pagination for Claim Ledger
  ...
  useEffect(() => {
    let mounted = true;
    async function load() {
      const d = await rewardDashboardsService.getStarterData();
      const c = await rewardDashboardsService.getStarterClaimHistory();
      if (mounted) {
        setData(d);
        setClaims(c);
  ...
  const handleConfirmClaim = async () => {
    ...
    try {
      ...
      if (res.success && data) {
        ...
        const updatedClaims = await rewardDashboardsService.getStarterClaimHistory();
        setClaims(updatedClaims);

        // Deduct MBTTC fee and update balances if callback available
        if (onUpdateRewards) {
          onUpdateRewards({
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-xs font-mono text-zinc-400">Loading Weekly Reward Starter...</span>
        </div>
      </div>
    );
  }
  ...
  const requiredDirects = data.requiredDirectPartners; // 2
  ...
  return (
    <div className="space-y-6 pb-20 animate-in fade-in duration-300">
      ...
      <div className="fixed top-24 right-1/4 w-96 h-96 bg-emerald-500/10 rounded-full blur-3xl pointer-events-none -z-10" />
      <div className="fixed bottom-1/3 left-1/4 w-80 h-80 bg-teal-500/10 rounded-full blur-3xl pointer-events-none -z-10" />

      {/* Hero Card: Cycle Status, Countdown & Claim Action */}
      <div className="relative rounded-3xl p-6 md:p-8 bg-gradient-to-br from-[#08120c] via-[#09150e] to-[#050b07] border border-emerald-500/35 shadow-[0_0_50px_rgba(16,185,129,0.15)] overflow-hidden">
        {/* Subtle decorative grid and highlight */}
        <div className="absolute top-0 right-0 w-80 h-80 bg-gradient-to-bl from-emerald-500/20 via-teal-500/10 to-transparent rounded-full blur-2xl pointer-events-none" />
        <div className="absolute top-0 left-12 w-48 h-1 bg-gradient-to-r from-transparent via-emerald-400/60 to-transparent" />

        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
          <div className="space-y-3">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-500/40 text-xs font-semibold text-emerald-300 shadow-[0_0_15px_rgba(16,185,129,0.2)]">
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
              <span>Week {data.currentWeek} Community Cycle • {data.status}</span>
            </div>

            <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-white flex items-center gap-3">
              Weekly Reward Starter
              <Award className="w-7 h-7 text-emerald-400 drop-shadow-[0_0_12px_#34d399]" />
            </h1>

            <p className="text-sm text-zinc-300 max-w-xl leading-relaxed">
              Automated community incentive engine distributing protocol volume pro-rata to verified members. Every direct partner gives exactly 1 share.
            </p>

            {/* Countdown bar & Starter Independent Smart Contract */}
            <div className="flex flex-wrap items-center gap-2.5">
              <div className="inline-flex items-center gap-3 px-4 py-2 rounded-2xl bg-black/60 border border-emerald-500/25">
                <Clock className="w-4 h-4 text-emerald-400 shrink-0" />
                <div className="flex items-center gap-2 text-xs">
                  <span className="text-zinc-400 font-medium">Starter Contract Timer:</span>
              ...
              {data.contractAddress && (
                <div className="inline-flex items-center gap-1.5 px-3 py-2 rounded-2xl bg-zinc-900/80 border border-emerald-500/20 text-[11px] font-mono text-zinc-400">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
                  <span className="text-zinc-300 font-semibold">{data.contractName || 'Starter Contract'}:</span>
                  <span className="text-emerald-400/90">{formatCompactAddress(data.contractAddress)}</span>
                </div>
              )}
            </div>
          </div>
          ...
          {/* Hero Claimable Box */}
          <div className="p-5 sm:p-6 rounded-2xl bg-zinc-950/90 border border-emerald-500/30 shadow-[0_0_30px_rgba(16,185,129,0.18)] flex flex-col justify-between min-w-[280px] lg:max-w-xs space-y-4">
            <div>
              <div className="flex items-center justify-between text-xs text-zinc-400 mb-1">
                <span>Claimable Reward</span>
                <span className="text-[10px] font-mono px-2 py-0.5 rounded-md bg-emerald-950/80 border border-emerald-500/40 text-emerald-300">
                  Ready to Claim
                </span>
              </div>
              <div className="flex items-center gap-2">
                <AnimatedDollarCoin size="md" />
                <span className="text-3xl sm:text-4xl font-black font-mono text-emerald-300 drop-shadow-[0_0_15px_rgba(52,211,153,0.35)]">
              ...
              <div className="text-[11px] text-zinc-400 mt-1 flex items-center justify-between">
                ...
                <span className="font-mono text-zinc-200 font-semibold inline-flex items-center gap-1">
                  <AnimatedDollarCoin size="xs" />
                  <span>{data.estimatedRewardUsdt.toFixed(2)}</span>
                </span>
              </div>
            </div>
              ...
              <button
                ...
                className={`w-full py-3 px-4 rounded-xl font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-2 transition-all duration-200 ${
                  (data.isClaimAvailable ?? (data.claimableRewardUsdt > 0)) && data.claimableRewardUsdt > 0
                    ? 'bg-gradient-to-r from-emerald-500 to-teal-400 hover:from-emerald-400 hover:to-teal-300 text-black shadow-[0_0_24px_rgba(16,185,129,0.4)] active:scale-[0.98] cursor-pointer'
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* KPI 1: Direct Partners & Shares */}
        <div className="p-5 rounded-2xl bg-[#080d0a]/90 border border-emerald-500/25 shadow-lg relative overflow-hidden">
          ...
          <div className="text-[11px] text-zinc-400 mt-2 flex items-center justify-between pt-2 border-t border-zinc-900">
            ...
            <span className="font-mono text-emerald-400 font-bold">{requiredDirects} Directs</span>
          </div>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-[#080d0a]/90 border border-emerald-500/25 shadow-lg relative overflow-hidden">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-2xl font-black font-mono text-emerald-300">{userShares}</span>
            <span className="text-xs text-zinc-400">Shares ({userShares} Directs)</span>
          </div>
          <div className="text-[11px] text-zinc-400 mt-2 flex items-center justify-between pt-2 border-t border-zinc-900">
            <span>Rule:</span>
            <span className="text-emerald-300 font-medium">1 Direct = 1 Share</span>
          </div>
        </div>

        {/* KPI 3: Qualification Status & Progress */}
        <div className="p-5 rounded-2xl bg-[#080d0a]/90 border border-emerald-500/25 shadow-lg relative overflow-hidden">
          <div className="absolute top-0 right-0 w-24 h-24 bg-emerald-500/10 rounded-full blur-xl pointer-events-none" />
          <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-baseline gap-2">
            <span className={`text-lg font-bold ${isQualified ? 'text-emerald-400' : 'text-amber-400'}`}>
              {isQualified ? 'Qualified' : 'Incomplete'}
            </span>
            <span className="text-xs text-zinc-400">({directPartnersCount} / {requiredDirects} Directs)</span>
          </div>
          <div className="text-[11px] text-zinc-400 mt-2 flex items-center justify-between pt-2 border-t border-zinc-900">
            <span>Progress:</span>
            <span className="font-mono text-emerald-400 font-bold">100% Qualified</span>
          </div>
        </div>

        {/* KPI 4: Live Pool Volume */}
        <div className="p-5 rounded-2xl bg-[#080d0a]/90 border border-emerald-500/25 shadow-lg relative overflow-hidden">
          <div className="absolute top-0 right-0 w-24 h-24 bg-amber-500/10 rounded-full blur-xl pointer-events-none" />
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-semibold text-zinc-400">Weekly Pool Volume</span>
          ...
          <div className="text-[11px] text-zinc-400 mt-2 flex items-center justify-between pt-2 border-t border-zinc-900">
            ...
            <span className="font-mono text-zinc-200 inline-flex items-center gap-1">
              <AnimatedDollarCoin size="xs" />
              <span>+{data.weeklyDepositsUsdt.toLocaleString()}</span>
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 rounded-3xl bg-[#080d0a]/90 border border-emerald-500/25 relative overflow-hidden shadow-xl">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-5 border-b border-zinc-800">
          <div>
            <h2 className="text-base font-bold text-white flex items-center gap-2">
              ...
              Qualification & Share Engine
            </h2>
            <p className="text-xs text-zinc-400 mt-0.5">
              Qualification requires 2 direct partners. Every direct partner gives exactly 1 share.
            </p>
          </div>

          <div className="flex items-center gap-3">
            <div className="px-3 py-1.5 rounded-xl bg-black/60 border border-emerald-500/30 text-xs font-mono">
              <span className="text-zinc-400 mr-2">Required Direct Partners:</span>
              <span className="text-emerald-400 font-bold">{requiredDirects}</span>
            </div>
            <div className="px-3 py-1.5 rounded-xl bg-emerald-950/70 border border-emerald-500/40 text-xs font-mono text-emerald-300">
          ...
          <div className="flex flex-col items-center justify-center p-5 rounded-2xl bg-zinc-950/60 border border-zinc-800/80">
            ...
            <div className="mt-3 text-center">
              <span className="text-xs font-bold text-emerald-400">
                {isQualified ? 'Qualified for Starter Pool' : 'Needs Directs'}
              </span>
              <p className="text-[11px] text-zinc-400 mt-0.5">
                {isQualified ? 'Threshold of 2 directs exceeded' : `Add ${requiredDirects - directPartnersCount} more to qualify`}
              </p>
            </div>
          </div>
          ...
          <div className="md:col-span-2 space-y-4">
            <div className="p-4 rounded-2xl bg-zinc-950/70 border border-zinc-800 space-y-2">
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="font-mono text-emerald-400 font-bold">1 Direct Partner = 1 Share</span>
              </div>
              <p className="text-xs text-zinc-400 leading-relaxed">
                Unlike tiered caps, every direct partner you sponsor earns you exactly 1 share continuously each week in the Starter Community Pool.
              </p>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2 text-center text-xs font-mono">
                ...
                <div className="p-2 rounded-xl bg-emerald-950/40 border border-emerald-500/30">
                  <span className="text-emerald-400/80 block text-[10px]">2 Directs (Req)</span>
                  <span className="font-bold text-emerald-300">2 Shares ★</span>
                </div>
                <div className="p-2 rounded-xl bg-zinc-900/60 border border-zinc-800">
                  <span className="text-zinc-400 block text-[10px]">5 Directs</span>
                  <span className="font-bold text-zinc-200">5 Shares</span>
                </div>
                <div className="p-2 rounded-xl bg-emerald-950/60 border border-emerald-500/40">
                  <span className="text-emerald-400/90 block text-[10px]">You: 12 Directs</span>
                  <span className="font-bold text-emerald-300">12 Shares</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              ...
              <div className="p-3 rounded-xl bg-zinc-950/60 border border-zinc-800">
                ...
                <span className="text-base font-black font-mono text-emerald-300">
                  {((userShares / data.totalWeeklyShares) * 100).toFixed(2)}%
                </span>
      ...
      {/* SVG Interactive Claim History Chart */}
      <RewardClaimChart
        data={rewardDashboardsService.getChartData('starter', 'ALL')}
        title="WEEKLY STARTER REWARD CLAIM HISTORY"
        themeColor="emerald"
      />

      {/* Transaction History Ledger */}
      <div className="p-4 sm:p-6 rounded-3xl bg-[#080d0a]/90 border border-emerald-500/25 shadow-xl space-y-4 min-w-0">
        <div className="flex items-center justify-between pb-3 border-b border-zinc-800">
          <div className="flex items-center gap-2">
            <Activity className="w-5 h-5 text-emerald-400" />
            <h3 className="text-base font-bold text-white">Weekly Starter Claim Ledger</h3>
          </div>
              ...
              {pagedClaims.map((row) => (
                <tr key={row.id} className="hover:bg-zinc-900/30 transition-colors">
                  ...
                  <td className="py-3 text-zinc-300">
                    <span className="text-emerald-400 font-bold">{row.directPartners}</span> Directs ({row.shares} Sh.)
                  </td>
                  <td className="py-3 font-bold text-emerald-400">
                    <span className="inline-flex items-center gap-1">
                      <AnimatedDollarCoin size="xs" />
                      <span>+{row.rewardUsdt.toFixed(2)}</span>
                    </span>
                  </td>
                  ...
                  <td className="py-3 text-right">
                    <div className="inline-flex items-center gap-1.5">
                      ...
                      <a
                        ...
                        className="p-1 rounded bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-emerald-400 transition-colors"
        ...
        <div className="md:hidden space-y-3 font-mono">
          {mobileClaimPairs.map((pair, pIdx) => (
            <div
              key={`starter-pair-${pair[0]?.id || pIdx}`}
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] text-zinc-400">
                        (<span className="text-emerald-400 font-semibold">{row.directPartners}</span> Directs / {row.shares} Sh.)
                      </span>
                    </div>
                    <div className="text-right">
                      <span className="font-bold text-xs text-emerald-400 inline-flex items-center gap-1">
                        <AnimatedDollarCoin size="xs" />
                        <span>+{row.rewardUsdt.toFixed(2)}</span>
                      </span>
                    </div>
      ...
      <RewardClaimModal
        ...
        title="Claim Weekly Starter Reward"
        subtitle="Week 14 Community Pool Distribution"
